...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self,lane,path,type,idx,created_at):
        super().__init__()
        # self.x=x
        # self.y=y
...
def generate_vehicles():
    ...
    while True:
        ...
        for line_dir in lines:
            ...
            vehicle = Vehicle(line_dir,vehicle_img_path_str,vehicle_type,len(generated_vehicles[line_dir]),sim_time)
            if(line_dir==0):
...
while running:
    ...
    e = time.time()
    max_upd_delay = max(max_upd_delay,e-b)
    pygame.time.delay(max(math.ceil(((1000/sim_rate)/30)-(e-b)),0))
